...
#define TAG "de.appwerft.audioselector"

using namespace v8;

static Persistent<Object> bindingCache;

static void Audioselector_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Audioselector.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::AudioselectorBindings::lookupGeneratedInit(
...
static void Audioselector_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Audioselector_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Audioselector_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::AudioselectorBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AudioselectorBinding = {
	"de.appwerft.audioselector",
	Audioselector_init,
	Audioselector_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_de_appwerft_audioselector_AudioselectorBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.audioselector", &AudioselectorBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::AudioselectorBindings::lookupGeneratedInit));
}
